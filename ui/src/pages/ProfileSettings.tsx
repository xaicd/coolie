...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("profile.breadcrumbSettings"), href: "/company/settings" },
      { label: t("profile.breadcrumbProfile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profile.failedUpdate"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profile.selectOrgBeforeAvatar"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profile.failedUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profile.failedRemoveAvatar"));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("profile.failedLoad")}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("profile.uploadHintWithCompany", { company: selectedCompany.name })
    : t("profile.uploadHintNoCompany");
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profile.avatarHelp", { hint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
        ...
        <Card className="space-y-4 rounded-(--rad-24) border-border/70 p-6">
          <div className="space-y-1">
            <h2 className="text-base font-semibold">{t("profile.languageTitle")}</h2>
            <p className="text-sm text-muted-foreground">
