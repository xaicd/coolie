...
**我们的代码只放 `main`。** 其他分支**不得**承载我们的工作:历史 `feature/*` 一旦内容进了 main 就是化石
(删除**不丢内容**,因为那些提交仍可从 main 到达),而"不在 main 里"的分支才是问题——
那是我们的代码躺在了 main 之外。这条**可检查**:下面 §7 那条报告会列出所有"内容不在 main 里"的分支。
